...
# type your code for the Exercise here, and Knit
ci <- ci %>% 
  mutate(lower_99 = lower - 0.02*(upper-lower)/(0.975*2),
         upper_99 = upper + 0.02*(upper-lower)/(0.975*2)) %>%
  mutate(capture_mu_99 = ifelse(lower_99 < params$mu & upper_99 > params$mu, "yes", "no"))
ci %>%
  group_by(capture_mu_99) %>%
  summarise (n = n()) %>%
  mutate(freq = n / sum(n))
