...
```{r gained-mature-ht}
# type your code for the Exercise here, and Knit
# use the median age as the cutoff for younger and mature mother
median(nc$mage)
```
...
nc_2 <- mutate(nc, age_cat = ifelse(mage>median(mage), "mature","younger"))
str(nc_2)
```
```{r cutoff-mature_original}
# type your code for Question 7 here, and Knit
g <- group_by(nc, mature)
g_age <- summarise(g,
                   max_age = max(mage),
                   min_age = min(mage)
                   )
...
```{r pick-your-own}
# type your code for the Exercise here, and Knit
# Are younger moms tends to visit hospitals less than mature moms?
inference(y = visits, x = age_cat, data = nc_2, statistic = "mean", type = "ht", null = 0, alternative = "less", method = "theoretical", order = c("younger","mature"))
```
